...
{
	int winSize = bufSize; // might be changed to a different value for overlapping
	...
	// float estimate_m[numBand], estimate_c[numBand];
	int i,j,k; // i - general counts; j - no. of chan; k - index of frames
	// int numBuf = numFrames / bufSize + 1;
	...
	{ 	
		...
		for (k = 0; k < numFrames; k+= hopSize)
		{
			...
			for (band_idx = 0; band_idx < numBand; band_idx++)
			{
				...
				// estimate_m[band_idx] = temp_m / bandWidth;
				// estimate_c[band_idx] = temp_c / bandWidth; 
				...
				for (i = band_idx * bandWidth; i < (band_idx+1) * bandWidth  ; i++)
				{
					...
					// y[winSize - 1 - i].Re 	=	 -y[i].Re;
					// y[winSize - 1 - i].Im 	=	 -y[i].Im;  
				}
			}
